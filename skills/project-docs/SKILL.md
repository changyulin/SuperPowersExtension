...
| 优先级 | 来源 | 写法要求 |
|---|---|---|
| 1 | 用户明确要求、验收标准、已确认决策 | 可写成确定事实，保留用户指定的词、列名、路径和格式 |
| 2 | 已验证代码行为、测试输出、命令输出 | 写明对应路径、命令或验证方式，不把未运行的验证写成已通过 |
| 3 | 现有 README、docs、PRD、ADR、模块文档 | 只复用当前任务仍适用的内容；发现冲突时标出冲突来源 |
| 4 | 从代码结构推断出的职责或边界 | 写成“从当前代码路径可见”，不要写成产品承诺或业务规则 |
| 5 | 无法确认的业务事实、性能指标、权限范围、安全结论 | 不写成事实；列为待确认问题 |
...
1. **收集输入**
   ...
   - 用 `rg --files -g "README.md" -g "*README.md" -g "docs/**" -g "PRD*.md" -g "ADR*.md"` 找入口文档和专题文档。
   - 若本次伴随代码变更，先查看变更文件、相关测试和用户验收要求，再写行为说明。
   - 输出：本次文档变更的目标、范围、事实来源清单。事实来源清单至少包含来源类型、文件路径或命令、可写入的结论。

2. **盘点现状**
   - 找出现有文档的角色：入口、总览、架构、规则、规范、决策、模块说明、操作指南。
   - 为候选文档标注：是否被 README 或导航引用、是否离本次变更最近、是否与代码事实冲突。
   - 标出缺口：缺入口、缺导航、缺边界、缺规则、缺验证方法、内容过期、链接失效。
   - 不先重命名、复制或重构目录。
...
3. **选择最小文档集**
   ...
   - 已有合适文档时直接更新它。
   - 没有承载位置时才新增文档，并补导航链接。
   - 如果同一事实会出现在多个文档，只在权威位置写完整内容，其他位置只放短说明和链接。

4. **写稳定内容**
   - 写目的、边界、职责、规则、例外、示例、验证方式、相关链接。
   - 写“为什么这么定”和“哪些地方不能改”。
   - 每段新增内容都要能追溯到“事实来源优先级”中的一项。
   - 对未确认事实使用“待确认”列表，不把假设混进正文。
   - 不复制代码中一眼可见的实现细节。

5. **CHECKPOINT: 新建或扩大文档范围前先停下**
   - 新建顶层目录、重排文档结构、移动或重命名已有文档前，先征求用户确认。
   - 准备新增 ADR 但决策未被用户或代码事实确认时，先征求用户确认。
   - 需要一次修改 3 个及以上文档、或会改变用户可见文档导航时，先列变更清单并等用户确认。
   - 需要补业务事实但来源不足时，先提出缺口，不编造规则。

6. **验收**
   - 检查 Markdown 相对链接目标是否存在、标题是否清楚、文档是否仍能独立理解。
   - 检查 README 或导航是否能到达新增文档。
   - 检查文档没有与已验证代码行为、测试结果或用户要求冲突。
   - 用 `rg -n "TODO|TBD|待确认|FIXME"` 检查占位内容；有意保留时在交付说明中点名。

7. **交付说明**
   - 说明更新了哪些文件。
   - 说明每类关键事实依据来自代码、现有文档、测试结果还是用户要求。
   - 说明执行过哪些文档验收检查，哪些检查未执行。
   - 说明未覆盖的缺口和需要用户确认的问题。
...
| 找不到事实来源 | 只写已确认事实和缺口清单 | 事实会影响接口、权限、性能、安全或业务规则时，先问用户 |
| 链接目标不存在 | 修正到真实路径或删除链接 | 目标文档尚未创建且需要确认时，先不加链接 |
| 需要批量调整多份文档 | 先列文件清单、每个文件的改动目的和依据 | 用户确认前只改最小入口文档 |
| 只有代码路径但没有验证结果 | 写成“当前代码路径显示”，并列出未运行的验证 | 用户要求验收结论时，先运行可用验证或说明无法验证 |
